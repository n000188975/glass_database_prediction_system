...
st.set_page_config(page_title='玻璃性質預測系統 (精簡輸入版)', layout='wide')
...
if data_source:
    ...
    display_labels = {k.replace('compounds_', ''): k for k in feature_cols}
    
    with tab1:
        st.header('1. 組分設定')
        # 使用多選選單讓使用者挑選要輸入的成分
        selected_labels = st.multiselect('請選擇要輸入含量的化學組分:', options=list(display_labels.keys()), default=['SiO2', 'Al2O3', 'B2O3'])
        ...
        if selected_labels:
            ...
            for i, label in enumerate(selected_labels):
                with cols[i % 4]:
                    user_input_vals[display_labels[label]] = st.number_input(f'{label} (wt%)', value=0.0, step=0.1, format='%.2f')
        
        if st.button('開始預測', use_container_width=True):
            # 初始化所有特徵為 0
            final_input = {c: 0.0 for c in feature_cols}
            # 更新使用者有輸入的部分
            for k, v in user_input_vals.items():
                final_input[k] = v
            ...
            if 'CTE' not in trained_models or 'E' not in trained_models:
                ...
                with st.spinner('正在分析最佳配方組合...'):
                    ...
                    if res.success or res.fun < 1.0:
                        ...
                        st.write('### 建議成分配比 (wt%)')
                        df_res = pd.DataFrame({'組分': list(display_labels.keys()), '含量 (wt%)': final_comp_vals})
                        st.dataframe(df_res[df_res['含量 (wt%)'] > 0.01].style.format({'含量 (wt%)': '{:.2f}'}), use_container_width=True)

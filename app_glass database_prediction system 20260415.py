...
st.set_page_config(page_title='LCTE玻璃開發系統', layout='wide')
...
if data_source:
    ...
    with tab1:
        ...
        if st.button('開始預測', use_container_width=True):
            ...
            if total > 0:
                ...
                st.subheader('預測結果')
                # 排除 DK 與 DF
                display_targets = {k: v for k, v in trained_models.items() if k not in ['Dk', 'Df']}
                res_cols = st.columns(len(display_targets))
                for i, (prop, model) in enumerate(display_targets.items()):
    ...
    with tab2:
        ...
        with col_a:
            cte_min = st.number_input('CTE 下限', value=2.4, step=0.01)
            cte_max = st.number_input('CTE 上限', value=2.6, step=0.01)
        with col_b:
            e_min = st.number_input('E 下限', value=80.0, step=0.5)
            e_max = st.number_input('E 上限', value=90.0, step=0.5)
        ...
        if st.button('啟動範圍優化搜尋', use_container_width=True):
            with st.spinner('正在分析最佳配方組合...'):
                ...
                res = differential_evolution(objective, bounds, tol=0.03, popsize=10, maxiter=70, seed=42)
                if res.success:
                    st.success('✅ 已找到建議配方！')
                    ...
                    m1, m2 = st.columns(2)
                    m1.metric('預計 CTE', f"{trained_models['CTE'].predict(X_final)[0]:.4f} ppm/K")
                    m2.metric('預計 E', f"{trained_models['E'].predict(X_final)[0]:.4f} GPa")
else:
    st.warning('請上傳資料庫檔案。')
